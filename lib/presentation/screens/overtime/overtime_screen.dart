...
class _OvertimeScreenState extends ConsumerState<OvertimeScreen> {
  ...
  StateNotifierProvider<OvertimeNotifier, OvertimeState> get _provider =>
      _approvals ? overtimeApprovalProvider : overtimeProvider;
  @override
  void initState() {
    super.initState();
  ...
  Future<void> _create() async {
    final success = await Navigator.push<bool>(
      context,
      MaterialPageRoute(builder: (_) => const CreateOvertimeScreen()),
    );
  ...
  Future<void> _cancel(OvertimeModel record) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('Cancel this request?'),
        content: Text(
          'The overtime request for ${record.date} will be cancelled.',
        ),
        actions: [
          TextButton(
  ...
  Widget _card(OvertimeModel record, bool busy) {
    final color = switch (record.status.toLowerCase()) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Overtime ${record.overtimeStart} – ${record.overtimeEnd} • ${record.hours.toStringAsFixed(2)} hours',
            ),
            ExpansionTile(
                ...
                Align(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Working Hours: ${record.workStart} – ${record.workEnd}',
                      ),
                      if (record.mealAllowance != null)
                        Text(
                          'Meal Allowance: ${NumberFormat.currency(locale: 'en_US', symbol: 'Rp ', decimalDigits: 0).format(record.mealAllowance)}',
                        ),
                      const SizedBox(height: 8),
  ...
  Future<void> _decide(OvertimeModel record, bool reject) async {
    var reason = '';
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          reject ? 'Reject Overtime Request' : 'Approve Overtime Request',
        ),
        scrollable: true,
        content: Form(
          key: formKey,
          child: Column(
            ...
            children: [
              Text(
                '${record.employeeName ?? 'Employee'} • ${record.date} • ${record.overtimeStart}',
              ),
    ...
    if (!mounted || confirmed != true) return;
    final notifier = ref.read(overtimeApprovalProvider.notifier);
      ...
      SnackBar(
        content: Text(
          success
              ...
              : ref.read(overtimeApprovalProvider).submitError ??
