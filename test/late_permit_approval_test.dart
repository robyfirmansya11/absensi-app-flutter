...
void main() {
    ...
    () async {
      ...
      client.dio.httpClientAdapter = ApiStub((request) {
        ...
        if (request.method == 'GET') {
          expect(request.path, '/late-working-permits/approvals');
        ...
        if (request.path.endsWith('/reject')) {
          ...
          expect(request.path, '/late-working-permits/1/approve');
          expect(request.data, isNull);
        }
        return jsonResponse({'data': approvalRow(1, actionable: false)});
      });
      final repo = LatePermitRepository(client);
  ...
  test(
    ...
    () async {
      ...
      final notifier = container.read(latePermitApprovalProvider.notifier);
      ...
      expect(container.read(latePermitApprovalProvider).records, isEmpty);
      release.complete();
      ...
      (tester) async {
        ...
        container.read(apiClientProvider).dio.httpClientAdapter = ApiStub((
          ...
          if (request.method == 'POST') {
            expect(
              request.path,
              '/late-working-permits/1/${reject ? 'reject' : 'approve'}',
            );
            if (reject) {
