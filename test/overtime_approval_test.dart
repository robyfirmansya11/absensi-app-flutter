...
Map<String, dynamic> approvalRow(int id, {bool actionable = true}) => {
  'id': id,
  'tanggal_lembur': '2026-09-23',
  'bulan_lembur': '2026-09',
  'mulai_kerja': '08:00',
  'selesai_kerja': '17:00',
  'mulai_lembur': '17:30',
  'selesai_lembur': '19:00',
  'jumlah_jam_lembur': 1.5,
  'uang_makan': 25000,
  'uraian_pekerjaan': 'Overtime work',
...
void main() {
    ...
    () async {
      ...
      client.dio.httpClientAdapter = ApiStub((request) {
        ...
        if (request.method == 'GET') {
          expect(request.path, '/overtime/approvals');
        ...
        if (request.path.endsWith('/reject')) {
          ...
          expect(request.path, '/overtime/1/approve');
          expect(request.data, isNull);
        }
        return jsonResponse({'data': approvalRow(1, actionable: false)});
      });
      final repo = OvertimeRepository(client);
  ...
  test(
    ...
    () async {
      ...
      final notifier = container.read(overtimeApprovalProvider.notifier);
      ...
      expect(container.read(overtimeApprovalProvider).records, isEmpty);
      release.complete();
      ...
      (tester) async {
        ...
        container.read(apiClientProvider).dio.httpClientAdapter = ApiStub((
          ...
          if (request.method == 'POST') {
            expect(
              request.path,
              '/overtime/1/${reject ? 'reject' : 'approve'}',
            );
            if (reject) {
