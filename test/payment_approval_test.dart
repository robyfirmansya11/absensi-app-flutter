...
import 'package:absensi_app_new/presentation/screens/payment/payment_screen.dart';
...
Map<String, dynamic> approvalRow(int id, {bool actionable = true}) => {
  ...
  'tanggal_penagihan': '2026-09-23',
  'tanggal_jatuhtempo': '2026-09-30',
  'no_invoice': 'INV-APP',
  'customer': 'Vendor',
  'jumlah': 100000,
  'ppn': 11000,
...
void main() {
    ...
    () async {
      ...
      client.dio.httpClientAdapter = ApiStub((request) {
        ...
        if (request.method == 'GET') {
          expect(request.path, '/payment-applications/approvals');
        ...
        if (request.path.endsWith('/reject')) {
          ...
          expect(request.path, '/payment-applications/1/approve');
  ...
  test(
    ...
    () async {
      ...
      final notifier = container.read(paymentApprovalProvider.notifier);
      ...
      expect(container.read(paymentApprovalProvider).records, isEmpty);
      release.complete();
      ...
      (tester) async {
        ...
        container.read(apiClientProvider).dio.httpClientAdapter = ApiStub((
          ...
          if (request.method == 'POST') {
            expect(
              request.path,
              '/payment-applications/1/${reject ? 'reject' : 'approve'}',
            );
            if (reject) {
        ...
        expect(find.textContaining('Total: Rp 111,500.00'), findsOneWidget);
