...
void main() {
    ...
    () async {
      ...
      client.dio.httpClientAdapter = ApiStub((request) {
        ...
        if (request.method == 'GET') {
          expect(request.path, '/stamp-applications/approvals');
        ...
        if (request.path.endsWith('/reject')) {
          ...
          expect(request.path, '/stamp-applications/1/approve');
  ...
  test(
    ...
    () async {
      ...
      final notifier = container.read(stampApprovalProvider.notifier);
      ...
      expect(container.read(stampApprovalProvider).records, isEmpty);
      release.complete();
      ...
      (tester) async {
        ...
        container.read(apiClientProvider).dio.httpClientAdapter = ApiStub((
          ...
          if (request.method == 'POST') {
            expect(
              request.path,
              '/stamp-applications/1/${reject ? 'reject' : 'approve'}',
            );
            if (reject) {
        ...
        expect(find.text('STAMP-001'), findsOneWidget);
