...
void main() {
    ...
    () async {
      ...
      client.dio.httpClientAdapter = ApiStub((request) {
        ...
        if (request.method == 'GET') {
          expect(request.path, '/loan-notes/approvals');
        ...
        if (request.path.endsWith('/reject')) {
          ...
          expect(request.path, '/loan-notes/1/approve');
  ...
  test(
    ...
    () async {
      ...
      final notifier = container.read(loanApprovalProvider.notifier);
      ...
      expect(container.read(loanApprovalProvider).records, isEmpty);
      release.complete();
      ...
      (tester) async {
        ...
        container.read(apiClientProvider).dio.httpClientAdapter = ApiStub((
          ...
          if (request.method == 'POST') {
            expect(
              request.path,
              '/loan-notes/1/${reject ? 'reject' : 'approve'}',
            );
            if (reject) {
