...
import 'package:absensi_app_new/presentation/screens/travel/travel_screen.dart';
...
void main() {
    ...
    () async {
      ...
      client.dio.httpClientAdapter = ApiStub((request) {
        ...
        if (request.method == 'GET') {
          expect(request.path, '/travel-reimbursements/approvals');
        ...
        if (request.path.endsWith('/reject')) {
          ...
          expect(request.path, '/travel-reimbursements/1/approve');
  ...
  test(
    ...
    () async {
      ...
      final notifier = container.read(travelApprovalProvider.notifier);
      ...
      expect(container.read(travelApprovalProvider).records, isEmpty);
      release.complete();
      ...
      (tester) async {
        ...
        container.read(apiClientProvider).dio.httpClientAdapter = ApiStub((
          ...
          if (request.method == 'POST') {
            expect(
              request.path,
              '/travel-reimbursements/1/${reject ? 'reject' : 'approve'}',
            );
            if (reject) {
        ...
        await tester.ensureVisible(find.text('Travel Details'));
        await tester.tap(find.text('Travel Details'));
        await tester.pumpAndSettle();
        expect(find.text('Office → Site'), findsOneWidget);
        expect(find.textContaining('Subtotal: Rp 415.50'), findsOneWidget);
